...
class Testopia(object):
    ...
    def product_check_by_id(self, product_id):
        return self.do_command("TestopiaProduct.check_product", [self._number_noop(product_id)])
    ...
    def user_lookup_login_by_id(self, id):
        ...
        """
        return self.do_command("TestopiaUser.lookup_login_by_id", [self._number_noop(id)])


    ############################## TestPlan ##################################
    
    def testplan_is_exist(self, product_id, plan_id):   
        tp_list = []
        if (type(plan_id) is int):      
            tp_list = self.testplan_list(plan_id=plan_id, product_id=product_id)  
        elif (type(plan_id) is str):        
            tp_list = self.testplan_list(name=plan_id, product_id=product_id)
        else:
            pass
        if (len(tp_list) > 0) :
            return True
    ...
    def testcase_get(self, case_id):
        ...
        if (type(case_id) is int):        
            ...
            return self.do_command("TestCase.get", [self._string_noop(case_id)])
        else:
            pass
        
    def testcase_is_exist(self, case_id):   
        tc_list = []
        if (type(case_id) is int):      
            tc_list = self.testcase_list(case_id=case_id)  
        elif (type(case_id) is str):        
            tc_list = self.testcase_list(alias=case_id)  
        else:
            pass
        if (len(tc_list) > 0) :
            return True
    ...
    def testcase_update(self, case_id, summary=None, isautomated=None,
        ...
        """
        if (type(case_id) is int):        
            return self.do_command("TestCase.update", [self._number_noop(case_id),
                    self._options_dict(
                       self._number_option('case_id', case_id),
                       self._string_option('summary', summary),
                       self._boolean_option('isautomated', isautomated),
            ...
            return self.do_command("TestCase.update", [self._string_noop(case_id),
                    self._options_dict(
                       #self._number_option('case_id', case_id),
                       self._string_option('summary', summary),
                       self._boolean_option('isautomated', isautomated),
    ...
    def testrun_add_cases(self, run_id, case_id):
        """Add a case to the given TestRun.

        'run_id' -- integer,
        'case_id' -- integer,

        Example: testrun_add_cases(10, 1)

        Result: The integer , 0, on success
        """

        if (type(case_id) is int):      
            return self.do_command("TestRun.add_cases", [self._number_noop(case_id),
                self._number_noop(run_id)
                   ])
        elif (type(case_id) is str):        
            return self.do_command("TestRun.add_cases", [self._string_noop(case_id),
                self._number_noop(run_id)
                   ])
        else:
            pass        
